# ============================================================
# IT Bot Market — Telegram бот для продажи IT-услуг
# ============================================================
# ВСЕ настройки берутся ИСКЛЮЧИТЕЛЬНО из переменных окружения (ENV).
# Установите их в панели хостинга BotHost (Environment Variables):
#
#   Обязательные:
#   BOT_TOKEN          — токен от @BotFather
#   ADMIN_ID           — ваш Telegram ID (число, например: 123456789)
#   CARD_DETAILS       — реквизиты карты для пополнения (текст)
#   REVIEWS_CHANNEL    — канал для отзывов (например: @my_reviews_channel)
#
#   Опциональные (если не заданы — используются значения по умолчанию):
#   DB_NAME            — путь к SQLite (по умолчанию: it_bot.db)
#   REFERRAL_PERCENT   — % реферального бонуса (по умолчанию: 10)
#   DEV_PERCENT        — % разработчику от заказа (по умолчанию: 70)
#   PREPAYMENT_LIMIT   — лимит для обязательной предоплаты (по умолчанию: 5000)
#
#   Зависимости: pip install python-telegram-bot aiosqlite
# ============================================================
...
# ==================== CONFIG (ТОЛЬКО ENV) ====================
# Все параметры берутся из переменных окружения (BotHost или другой хостинг)
# Обязательные переменные:
#   BOT_TOKEN          — токен бота от @BotFather
#   ADMIN_ID           — Telegram ID главного администратора
#   CARD_DETAILS       — реквизиты карты для пополнения
#   REVIEWS_CHANNEL    — канал для публикации отзывов (например, @my_reviews)
#   DB_NAME            — путь к SQLite БД (по умолчанию it_bot.db)
#   REFERRAL_PERCENT   — процент реферального вознаграждения (по умолчанию 10)
#   DEV_PERCENT        — процент разработчику (по умолчанию 70)
#   PREPAYMENT_LIMIT   — лимит для предоплаты (по умолчанию 5000)
...
if missing:
    raise EnvironmentError(
        f"[CONFIG ERROR] Отсутствуют обязательные переменные окружения: {', '.join(missing)}. "
        f"Установите их в панели хостинга BotHost (Environment Variables) перед запуском."
    )

try:
    ADMIN_ID = int(ADMIN_ID)
except ValueError:
    raise EnvironmentError("[CONFIG ERROR] ADMIN_ID должен быть целым числом (Telegram ID).")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'order_desc':
        ...
        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=f"Клиент предложил цену {price} ₽ для заказа #{order_id}.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("Принять цену", callback_data=f"admin_acceptnegoprice_{order_id}_{price}")],
...
Текст: {text}"
        ...
        parts = text.split(',')
        if len(parts) != 3:
            await update.message.reply_text("Неверный формат. Введите через запятую.")
            return
        try:
        ...
        try:
            ...
            await update.message.reply_text("Неверные числа.")
            return
...
"
        ...
        if tg_id == ADMIN_ID:
            kb = InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("👥 Управление персоналом", callback_data="admin_staff")],
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("platform_"):
        ...
        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=f"Заказ #{order_id} подтвержден клиентом. Назначьте разработчика.",
            reply_markup=InlineKeyboardMarkup([
        ...
        await query.edit_message_text("Цена обновлена и отправлена клиенту.")
    elif data.startswith("admin_rejectnegoprice_"):
        order_id = int(data.split('_')[2])
        await query.edit_message_text(f"Заказ #{order_id}: цена отклонена. Ожидайте новую цену от админа.")
...
"
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("Назначить разработчика", callback_data="staff_setdev")],
            [InlineKeyboardButton("Снять разработчика", callback_data="staff_removedev")],
            [InlineKeyboardButton("Забанить", callback_data="staff_ban")],
            [InlineKeyboardButton("Разбанить", callback_data="staff_unban")],
        ])
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'admin_send_zip' and tg_id == ADMIN_ID:
        ...
        await update.message.reply_text("ZIP отправлен клиенту.")
    else:
        await update.message.reply_text("Документ не ожидался.")
